...
- **Líneas**: Múltiples referencias a `lovenda` y `mywed360` son intencionales
...
- `src/hooks/useGuests.js`: `mywed360Guests`
- `src/hooks/useSpecialMoments.js`: `mywed360SpecialMoments`
- `src/hooks/useUserCollection.js`: `mywed360User_`
- `src/hooks/useWeddingCollection.js`: `mywed360-${wid}-${name}`
...
```javascript
// PROBLEMA:
¡Tu dirección de correo <strong>{currentUsername}@mywed360</strong>
...
**4. `render.yaml`**
```javascript
name: mywed360-backend  // Debería ser maloveapp-backend
```

**5. `public/enable-auth.html`**
```javascript
authDomain: "mywed360.firebaseapp.com"  // Debería actualizarse
```
...
- [ ] Corregir `@mywed360` en `src/pages/EmailSetup.jsx`
