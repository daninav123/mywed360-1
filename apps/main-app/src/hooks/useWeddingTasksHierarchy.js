...
export default function useWeddingTasksHierarchy(weddingId) {
  ...
  useEffect(() => {
    ...
    const unsubParents = onSnapshot(
      ...
      (error) => {
        // Silenciar errores de permisos - mantener estado actual
        // console.warn('[useWeddingTasksHierarchy] Error cargando tasks:', error);
        setParents([]);
      }
    );
