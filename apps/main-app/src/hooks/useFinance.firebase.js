...
export default function useFinance() {
  ...
  const [contributions, setContributions] = useState({
    ...
    giftPerGuest: 0,
    guestCount: 0,
    initialBalance: 0, // Saldo inicial disponible
    balanceAdjustments: [], // Historial de ajustes: [{ amount, date, reason }]
  });
  ...
  // Cargar y sincronizar datos de finance/main desde Firestore
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.warn('[useFinance] Error listening to finance/main:', error);
      }
    );
  ...
  const stats = useMemo(
    () => ({
      ...
      initialBalance: contributions.initialBalance || 0,
  ...
  useEffect(() => {
    ...
    const applyLocalFinance = () => {
      ...
      setContributions({
        ...
        initialBalance: parseMoneyValue(localContributions.initialBalance, 0),
        balanceAdjustments: Array.isArray(localContributions.balanceAdjustments)
          ? localContributions.balanceAdjustments.map((adj) => ({
              amount: parseMoneyValue(adj?.amount, 0),
              date: adj?.date ? String(adj.date) : new Date().toISOString(),
    ...
    unsub = onSnapshot(
      ...
      (snap) => {
        ...
        setContributions({
          ...
          initialBalance: parseMoneyValue(contributionsData.initialBalance, 0),
          balanceAdjustments: Array.isArray(contributionsData.balanceAdjustments)
            ? contributionsData.balanceAdjustments.map((adj) => ({
                amount: parseMoneyValue(adj?.amount, 0),
                date: adj?.date ? String(adj.date) : new Date().toISOString(),
                reason: adj?.reason ? String(adj.reason) : '',
              }))
