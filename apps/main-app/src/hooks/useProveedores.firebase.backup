    ...
    async (providerId, lineData) => {
      ...
      try {
        const payload = {
          ...
          categoryKey: lineData?.categoryKey || null,
          assignedBudget: typeof lineData?.assignedBudget === 'number' ? lineData.assignedBudget : (typeof lineData?.budget === 'number' ? lineData.budget : 0),
          status: lineData?.status || 'Pendiente',
          notes: lineData?.notes || '',
          deliverables: Array.isArray(lineData?.deliverables) ? lineData.deliverables : [],
          milestones: Array.isArray(lineData?.milestones) ? lineData.milestones : [],
  ...
  const addProvider = useCallback(
    async (providerData) => {
      ...
      try {
        ...
        if (newProvider.paymentSchedule && Array.isArray(newProvider.paymentSchedule) && newProvider.paymentSchedule.length > 0) {
          console.log('[useProveedores] Nuevo proveedor con plan de pagos, programando sincronización');
          // La sincronización se hará cuando useFinance esté disponible
          // Por ahora solo guardamos el provider con su paymentSchedule
        }
