...
export default function TransactionRow({
  ...
  return (
    <tr className="hover:bg-[var(--color-accent-10)]">
          ...
          <div className="mt-1 text-xs text-[color:var(--color-text-60)] space-y-0.5">
            ...
            {tx.dueDate && (
              <div className={isOverdue ? 'text-[color:var(--color-danger)]' : ''}>
                {tr('finance.transactions.dueOn', { defaultValue: 'Vence:' })}{' '}
        ...
        {Array.isArray(tx.attachments) && tx.attachments.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-2 text-xs text-[color:var(--color-text-70)]">
            {tx.attachments.map((att, index) => {
              const label =
                att?.filename ||
                `${tr('finance.transactions.attachment', { defaultValue: 'Adjunto' })} ${index + 1}`;
              return att?.url ? (
                <a
                  key={att.url || index}
                  href={att.url}
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm">
        ...
        >
          {tx.type === 'income'
            ...
            : tr('finance.transactions.expense', { defaultValue: 'Gasto' })}
        </span>
      </td>
      <td className="px-6 py-4 whitespace-nowrap text-sm">
        <span
      ...
      <td
        className={`px-6 py-4 whitespace-nowrap text-sm font-medium text-right ${tx.type === 'income' ? 'text-[color:var(--color-success)]' : 'text-[color:var(--color-danger)]'}`}
      >
        {tx.type === 'income' ? '+' : '-'}
