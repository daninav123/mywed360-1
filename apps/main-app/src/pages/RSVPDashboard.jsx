...
export default function RSVPDashboard() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {!stats ?(
        <div className="" className="text-secondary">Sin datos de respuestas todavía.</div>
      ) : (
        <>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div className="border rounded p-4 bg-surface">
              <div className="text-sm " className="text-muted">Invitaciones</div>
              <div className="text-2xl font-bold">{stats.totalInvitations || 0}</div>
            </div>
            <div className="border rounded p-4 bg-surface">
              <div className="text-sm " className="text-muted">Respuestas</div>
              <div className="text-2xl font-bold">{stats.totalResponses || 0}</div>
            </div>
            <div className="border rounded p-4 bg-surface">
              <div className="text-sm " className="text-muted">Asistentes Confirmados</div>
              <div className="text-2xl font-bold">{stats.confirmedAttendees || 0}</div>
            </div>
            <div className="border rounded p-4 bg-surface">
              <div className="text-sm " className="text-muted">Rechazadas</div>
              <div className="text-2xl font-bold">{stats.declinedInvitations || 0}</div>
            </div>
            <div className="border rounded p-4 bg-surface">
              <div className="text-sm " className="text-muted">Pendientes</div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="border rounded p-4 bg-surface">
              ...
              <div className="flex justify-between text-xs  mt-2" className="text-muted">
              ...
              {computedMetrics.avgMinutes != null && (
                <p className="text-xs  mt-3" className="text-muted">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-7 gap-3">
                  {timelineData.map((point) => (
                    <div key={point.label} className="flex flex-col items-center">
                      <div className="w-full h-24  rounded relative overflow-hidden" className="bg-page">
                        <div
                      ...
                      <div className="text-xs  mt-1 text-center" className="text-muted">{point.label}</div>
                      <div className="text-xs font-medium " className="text-body">
            ...
            <div className="border rounded p-4 bg-surface">
              ...
              {channelBreakdown.length === 0 ? (
                <div className="text-sm " className="text-secondary">Sin datos por canal todavía.</div>
              ) : (
                <ul className="space-y-1 text-sm">
                  {channelBreakdown.map((item) => (
                    <li key={item.channel} className="flex justify-between">
                      <span className="capitalize " className="text-secondary">{item.channel}</span>
                      <span className="font-medium " className="text-body">
          ...
          <div className="border rounded p-4 bg-surface">
            ...
            {loadingPending ?(
              <div className="" className="text-secondary">Cargando pendientes…</div>
            ) : pendingGuests.length === 0 ?(
              <div className="" className="text-secondary">Sin pendientes ahora mismo.</div>
            ) : (
