...
export default function SpecialElementsPanel({ onAdd }) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        <h3 className="text-sm font-semibold text-gray-900 mb-2">Elementos Especiales</h3>
        ...
        {specialElements.map((element) => {
          ...
          return (
            <button
              ...
              className="aspect-square bg-gray-50 rounded-lg hover:bg-gray-100 transition-all hover:ring-2 hover:ring-blue-500 p-3 flex flex-col items-center justify-center gap-1"
              title={element.name}
            >
              <Icon className="w-8 h-8 text-gray-700" />
              <span className="text-xs text-gray-600 text-center">{element.name}</span>
            </button>
          );
        })}
      </div>
