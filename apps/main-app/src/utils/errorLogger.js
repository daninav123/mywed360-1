  ...
  */
    ...
    try {
      ...
      if (!backendUrl) {
        this.diagnostics.openai = {
          ...
          details: {
            source: 'backend',
            message: 'Backend no disponible para diagnóstico de OpenAI',
          },
        };
        return;
      }
      ...
      try {
        const response = await fetch(`${backendUrl}/api/diagnostic/status?x-suppress-error-logging=1`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
        ...
        if (!response.ok) {
          throw new Error(`Diagnostic status respondió con ${response.status}`);
        }
        ...
        const configured = Boolean(payload?.data?.environment_variables?.openai?.api_key);
  ...
  async checkMailgunConnection() {
    try {
      ...
      // Verificar a través del backend si está disponible (preferido por seguridad)
      const backendUrl = import.meta.env.VITE_BACKEND_BASE_URL;
      if (backendUrl) {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 10000);
        ...
        let response;
        try {
          response = await fetch(`${backendUrl}/api/mailgun/test?x-suppress-error-logging=1`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
              'X-Suppress-Error-Logging': '1',
        ...
        // Si no hay backend, no podemos verificar Mailgun sin exponer keys en cliente.
        this.diagnostics.mailgun = {
          status: 'warning',
          details: {
            message: 'Backend no disponible para test de Mailgun',
            domain: domain || null,
          },
        };
      }
      ...
      this.diagnostics.mailgun = {
        ...
        details: {
          error: error.message,
          hasBackend: !!import.meta.env.VITE_BACKEND_BASE_URL,
