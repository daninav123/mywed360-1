  ...
  async testMailgunConfig() {
    try {
      ...
      if (!domain) {
        return {
          status: 'warning',
          message: 'MAILGUN_DOMAIN no configurado en cliente (se gestiona desde backend)',
          details: { hasDomain: false },
        };
      }
      ...
      if (this.backendUrl) {
        ...
        let response;
        try {
          response = await fetch(`${this.backendUrl}/api/mailgun/test?x-suppress-error-logging=1`, {
            method: 'GET',
            headers: { 'X-Suppress-Error-Logging': '1' },
        ...
        if (!response.ok) {
          ...
          try {
            response = await fetch(`${this.backendUrl}/api/test/mailgun?x-suppress-error-logging=1`, {
              method: 'GET',
              headers: { 'X-Suppress-Error-Logging': '1' },
        ...
        if (response.ok) {
          ...
          return {
            ...
            details: { error, domain, hasDomain: !!domain },
          };
        }
      }
      ...
      return {
        status: 'warning',
        message: 'Backend no disponible para verificar Mailgun',
