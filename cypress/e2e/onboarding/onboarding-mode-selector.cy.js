...
describe('Onboarding - Selector de modo', () => {
  beforeEach(() => {
    ...
    cy.window().then((win) => {
      ...
      win.localStorage.removeItem('mywed360_cached_weddings');
      win.localStorage.removeItem('mywed360_active_wedding');
      win.localStorage.removeItem('lovenda_active_wedding');
      win.localStorage.removeItem('mywed360_active_wedding_name');
  ...
  it('permite elegir el tutorial clásico desde el selector', () => {
    ...
    cy.contains('Bienvenido a MyWed360', { timeout: 15000 }).should('be.visible');
