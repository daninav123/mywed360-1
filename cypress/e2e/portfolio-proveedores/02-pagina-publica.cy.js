...
describe('Portfolio Proveedores - Página Pública', () => {
  ...
  it('Debe mostrar el portfolio de fotos', () => {
    ...
    cy.get('[data-testid="portfolio-grid"]', { timeout: 10000 }).should('exist');

    // Verificar que muestra las fotos
    mockPortfolio.forEach((photo) => {
      cy.contains(photo.title).should('be.visible');
    });

    // Verificar que las fotos destacadas tienen badge
    cy.contains('Destacada').should('exist');
  });
