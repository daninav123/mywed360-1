...
export default function RFQModal({ open, onClose, providers = [], defaultSubject = 'Solicitud de presupuesto', defaultBody = '' }) {
  const { activeWedding } = useWedding();
  const [subject, setSubject] = useState(defaultSubject);
  const [body, setBody] = useState(defaultBody || 'Hola,\n\nNos gustaría recibir un presupuesto detallado para nuestro evento. Por favor, incluye condiciones, logística y extras.\n\nGracias.');
  ...
  const variables = useMemo(() => {
    ...
    const dateStr = (() => {
      try { const d = typeof dateVal?.toDate === 'function' ? dateVal.toDate() : new Date(dateVal); return d.toLocaleDateString('es-ES'); } catch { return String(dateVal || ''); }
    })();
    return {
    ...
    try {
      ...
      for (const p of targets) {
        try {
          ...
          const res = await apiPost('/api/mail', { to: p.email, subject: compiledSubject, body: compiledBody, attachments: attachments.map(a=>({ filename: a.name || a.filename, url: a.url })) }, { auth: true });
          if (!res.ok) {
          ...
          try {
            if (activeWedding && p.id && !String(p.id).startsWith('web-')) {
              const col = collection(db, 'weddings', activeWedding, 'suppliers', p.id, 'rfqHistory');
              await addDoc(col, {
  ...
  const saveCurrentAsTemplate = async () => {
    const id = await saveTemplate({ id: tplId || undefined, name: tplName || 'RFQ', service: tplService || '', subject, body });
    setTplId(id);
  };
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <div>
            ...
            <select className="border rounded p-2 w-full" value={tplId} onChange={(e)=>applyTemplate(e.target.value)}>
              <option value="">Seleccionar plantilla</option>
              {templates
                .filter(t => !tplService || !t.service || t.service === tplService)
                .map(t => (
                  <option key={t.id} value={t.id}>{t.name}{t.service?` (${t.service})`:''}</option>
                ))}
            </select>
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Nombre plantilla</label>
            <input className="border rounded p-2 w-full" value={tplName} onChange={(e)=>setTplName(e.target.value)} placeholder="Ej. RFQ Fotografía" />
          </div>
          <div>
            <label className="block text-xs text-gray-500 mb-1">Servicio</label>
            <input className="border rounded p-2 w-full" value={tplService} onChange={(e)=>setTplService(e.target.value)} placeholder="Ej. Música" />
          </div>
          <div className="md:col-span-3 flex gap-2 justify-end">
            <Button size="sm" variant="outline" onClick={saveCurrentAsTemplate}>Guardar plantilla</Button>
            {tplId && <Button size="sm" variant="outline" onClick={async ()=>{ await removeTemplate(tplId); setTplId(''); }}>Eliminar plantilla</Button>}
          </div>
        </div>
        ...
        <div>
          ...
          <input ref={subjectRef} onFocus={()=>setActiveField('subject')} className="w-full border rounded p-2" value={subject} onChange={(e) => setSubject(e.target.value)} />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1">Mensaje</label>
          <textarea ref={bodyRef} onFocus={()=>setActiveField('body')} className="w-full border rounded p-2" rows={8} value={body} onChange={(e) => setBody(e.target.value)} />
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {['{proveedor_nombre}','{proveedor_servicio}'].map((v) => (
                <span key={v} className="text-xs px-2 py-1 bg-gray-100 rounded">{v}</span>
              ))}
            </div>
          </div>
          <div>
            ...
            <div className="space-y-2">
              {attachments.map((a, idx) => (
                <div key={idx} className="flex gap-2">
                  <input className="border rounded p-1 flex-1" placeholder="Nombre" value={a.name||''} onChange={(e)=>{
                    const next = attachments.slice(); next[idx] = { ...next[idx], name: e.target.value }; setAttachments(next);
                  }} />
                  <input className="border rounded p-1 flex-1" placeholder="https://..." value={a.url||''} onChange={(e)=>{
                    const next = attachments.slice(); next[idx] = { ...next[idx], url: e.target.value }; setAttachments(next);
                  }} />
                  <Button size="sm" variant="outline" onClick={()=>setAttachments(attachments.filter((_,i)=>i!==idx))}>Quitar</Button>
                </div>
              ))}
              <div className="flex items-center gap-2">
                <Button size="sm" variant="outline" onClick={()=>setAttachments([...attachments,{ name:'', url:'' }])}>Añadir URL</Button>
                ...
                <Button size="sm" variant="outline" onClick={()=>fileInputRef.current?.click()} disabled={!activeWedding || uploading}>
        ...
        {result && (
          <div className={`text-sm ${result.ok ? 'text-green-700 bg-green-50 border-green-200' : 'text-red-700 bg-red-50 border-red-200'} border p-2 rounded`}>
            {result.ok ? 'Enviado correctamente.' : (
              <>
                Fallos: {result.errors.length}
                <ul className="list-disc ml-5">
                  {result.errors.map((e, i) => (
                    <li key={i}>{e.email}: {e.err}</li>
                  ))}
                </ul>
              </>
            )}
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={handleSend} disabled={sending || targets.length === 0}>{sending ? 'Enviando…' : 'Enviar'}</Button>
        </div>
      </div>
    </Modal>
  );
