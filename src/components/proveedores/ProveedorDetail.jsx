...
/**
 * Panel de detalle del proveedor (modal centrado)
 */
const ProveedorDetail = ({ provider, onClose, onEdit, activeTab, setActiveTab, onOpenGroups }) => {
  const [rating, setRating] = useState(provider.ratingCount > 0 ? provider.rating / provider.ratingCount : 0);
  const { activeWedding } = useWedding();
    ...
    <div className="flex">
      {[1, 2, 3, 4, 5].map((star) => (
        <Star
          key={star}
          size={interactive ? 24 : 20}
          className={`${star <= currentRating ? 'text-yellow-500 fill-yellow-500' : 'text-gray-300'} ${interactive ? 'cursor-pointer' : ''}`}
          onClick={() => interactive && setRating(star)}
        />
      ))}
    </div>
    ...
    <>
      <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          {/* Tabs */}
          <div className="flex border-b">
            <button className={`py-2 px-4 ${activeTab === 'info' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`} onClick={() => setActiveTab('info')}>Información</button>
            <button className={`py-2 px-4 ${activeTab === 'communications' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`} onClick={() => setActiveTab('communications')}>Comunicaciones</button>
            <button className={`py-2 px-4 ${activeTab === 'tracking' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500'}`} onClick={() => setActiveTab('tracking')}>Seguimiento</button>
          </div>
          ...
          <div className="overflow-y-auto p-4 flex-1">
            {activeTab === 'info' && (
              <div className="space-y-6">
                <Card>
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center flex-wrap gap-2">
                      <span className={`text-sm px-3 py-1 rounded-full ${getStatusColor(provider.status)}`}>{provider.status}</span>
                      <span className="ml-2 text-gray-500">{provider.service}</span>
                      {!!provider.groupName && !groupCleared && (
                        <button
                          type="button"
                  ...
                  <div className="space-y-3 mt-4">
                    ...
                    {provider.email && (
                      <div className="flex items-center">
                        ...
                        <div className="overflow-hidden">
                          <p className="font-medium">Email</p>
                          <a href={`mailto:${provider.email}`} className="text-sm text-blue-600 hover:underline truncate block">{provider.email}</a>
                        </div>
                      </div>
                    )}
                    {provider.website && (
                      <div className="flex items-center">
                        ...
                        <div className="overflow-hidden">
                          ...
                          <a href={provider.website} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline truncate block">{provider.website}</a>
                        </div>
                      </div>
                    )}
                    {provider.date && (
                  ...
                  <div className="mt-6">
                    ...
                    <div className="flex items-center space-x-4">
                      {renderRatingStars(rating, true)}
                      <span className="text-sm text-gray-500">{rating.toFixed(1)} de 5 ({provider.ratingCount || 0} valoraciones)</span>
                    </div>
                  </div>
                </Card>
                ...
                <Card>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button onClick={handleOpenLegalDocs} variant="outline">Crear contrato en Documentos</Button>
                      <Button onClick={handleGenerateContract} disabled={!activeWedding || generating}>{generating ? 'Generando…' : 'Generar ahora'}</Button>
                    </div>
                  </div>
                </Card>
            ...
            {activeTab === 'tracking' && (
              <div className="space-y-4">
                <Card>
                  <h3 className="text-lg font-medium mb-3">Seguimiento</h3>
                  <p className="text-gray-500">Historial de seguimiento de comunicaciones con este proveedor.</p>
                </Card>
              </div>
            )}
          </div>
