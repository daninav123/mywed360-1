...
export default function AISearchModal({
  ...
  const handleSubmit = useCallback(
    (e) => {
      ...
      onSearch?.(query, { service: serviceFilter || undefined });
    },
    [query, onSearch, serviceFilter]
  );
  ...
  const handleQueryChange = useCallback(
    (e) => {
      ...
      if (value.trim().length > 2) {
        const t = setTimeout(() => onSearch?.(value, { service: serviceFilter || undefined }), 500);
        setSearchTimeout(t);
      }
    },
    [searchTimeout, onSearch, serviceFilter]
  );

  const selectQuery = (q) => {
    setQuery(q);
    onSearch?.(q, { service: serviceFilter || undefined });
  };
