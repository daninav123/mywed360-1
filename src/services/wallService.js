...
const DEMO_IMAGES = [
  {
    ...
    tags: [i18n.t('common.decoracion'), 'flores'],
    source: 'demo',
  },
  {
    id: 'demo_2',
  ...
  },
  {
    ...
    tags: ['flores', 'vestido'],
    source: 'demoi18n.t('common.obtiene_lote_paginado_del_muro_inspiracion')https://maloveapp-backend.onrender.com';
...
export async function fetchWall(page = 1, query = 'wedding') {
  ...
  const normalize = (p) => {
    ...
    if (!obj.tags || obj.tags.length === 0) {
      const txt = (p.description || p.alt || p.permalink || 'i18n.t('common.tostring_const_inferred_guesstagstxt_inferredlength_objtags')wallService: usando datos demo (circuit breaker activo)');
      return DEMO_IMAGES;
    }
  }
  ...
  try {
    ...
    data = data.filter((p) => {
      const origBlocked = /(pinimg|pinterest)\./i.test(p.original_url || 'i18n.t('common.const_proxiedok_purl_purlstartswithapibaseapiimageproxy_dominio_bloqueado')wallService: endpoint no disponible, usando datos demo');
    return DEMO_IMAGES;
  }
}
