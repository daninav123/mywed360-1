...
function generateTestData(type, size = 'medium') {
    ...
    case 'providers': {
      ...
      for (let i = 0; i < provCount; i++) {
        result.push({
          ...
          type: ['Catering', 'Fotografía', 'Flores', 'Música', 'Decoración'][
  ...
  console.log(
    `✅ Generados ${result.length} elementos en ${((endTime - startTime) / 1000).toFixed(2)} segundos`
  );
...
async function testGlobalSearch(params) {
  ...
  console.log(
    `Probando búsqueda con ${emails.length} emails, ${events.length} eventos, ${providers.length} proveedores`
  );
