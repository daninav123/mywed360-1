...
function loadLegalProgress(weddingId) {
  try { return JSON.parse(localStorage.getItem(LEGAL_LS_KEY(weddingId)) || '{}'); } catch { return {}; }
}
function saveLegalProgress(weddingId, data) {
  try { localStorage.setItem(LEGAL_LS_KEY(weddingId), JSON.stringify(data || {})); } catch {}
}
...
const LEGAL_REQUIREMENTS = {
  ...
  FR: { civil: ['Pièces d’identité', 'Actes de naissance récents'], iglesia: ['Certificat de baptême', 'Certificat de confirmation'] },
  US: { civil: ['ID válidos', 'Solicitud de Marriage License'], iglesia: ['Baptismal certificate', 'Pre-Cana course'] },
};
...
const DOWNLOAD_TEMPLATES = {
  ...
  FR: {
    civil: [ { id: 'solicitud_expediente_matrimonial', title: 'Demande de dossier matrimonial (modèle)', desc: 'Modèle générique' } ],
    iglesia: [ { id: 'solicitud_expediente_canonico', title: 'Demande de dossier canonique (modèle)', desc: 'Modèle générique' } ],
  },
  US: {
    civil: [ { id: 'solicitud_expediente_matrimonial', title: 'Marriage License Application (template)', desc: 'Generic template' } ],
    iglesia: [ { id: 'solicitud_expediente_canonico', title: 'Canonical Marriage Dossier Request (template)', desc: 'Generic template' } ],
  },
};
...
async function downloadAsPdf(filename, html) {
  try {
    ...
    try { const h2c = await import('html2canvas'); if (typeof window !== 'undefined') window.html2canvas = h2c.default || h2c; } catch {}
    const jsPDF = jspdfMod.default || jspdfMod.jsPDF || jspdfMod;
    const container = document.createElement('div');
    container.style.position = 'fixed'; container.style.left = '-10000px'; container.style.top = '0'; container.style.width = '800px';
    container.innerHTML = html; document.body.appendChild(container);
    const doc = new jsPDF({ unit: 'pt', format: 'a4' });
    await doc.html(container, { x: 32, y: 32, width: 531, windowWidth: 800, html2canvas: { scale: 0.9 } });
...
export default function DocumentosLegales() {
  ...
  const tr = (key, def) => { try { const v = t(key); return v === key ? def : v; } catch { return def; } };
      ...
      {activeWedding && (
        <>
          ...
          <Card className="p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="inline-flex rounded overflow-hidden border">
                  <button className={`${legalType==='civil' ? 'bg-blue-600 text-white' : 'bg-white'} px-3 py-1 text-sm`} onClick={() => setLegalType('civil')}>Civil / Juzgado</button>
                  <button className={`${legalType==='iglesia' ? 'bg-blue-600 text-white' : 'bg-white'} px-3 py-1 text-sm`} onClick={() => setLegalType('iglesia')}>Iglesia</button>
                </div>
                <label className="text-sm text-gray-600 ml-2">País:</label>
                <select className="border rounded px-2 py-1 text-sm" value={form.region} onChange={(e)=> setForm((f)=>({ ...f, region: e.target.value }))}>
            ...
            <p className="text-sm text-gray-600 mb-3">Esta lista es orientativa y puede variar según municipio, Registro Civil o diócesis. Confirma siempre con tu oficina/parroquia.</p>
                      ...
                      <input
                        ...
                        onChange={(e)=>{
                          ...
                          if (e.target.checked) {
                            ...
                            if (typeof prev === 'object' && prev.file) { next[key] = { ...prev, done: false }; }
                            else { delete next[key]; }
                          }
                          setLegalProgress(next); saveLegalProgress(activeWedding, next);
                        }}
                      />
                      <span>{item}</span>
                    </label>
                    <div className="pl-6 flex items-center gap-2 text-xs text-gray-600">
                      <input
                        ...
                        onChange={async (e)=>{
                          ...
                          const upKey = key; setUploadingReq((u)=>({ ...u, [upKey]: true }));
                          try {
                            ...
                            setLegalProgress(next); saveLegalProgress(activeWedding, next);
                            try { toast.success(tr('documents.uploaded','Archivo subido')); } catch {}
                          } catch (err) { console.warn('Upload requirement failed', err); try { toast.error(tr('documents.uploadFailed','No se pudo subir el archivo')); } catch {} }
                          finally { setUploadingReq((u)=>({ ...u, [upKey]: false })); e.target.value = ''; }
                        }}
                      />
                      ...
                      {fileMeta && (
                        <>
                          {fileMeta.url ? <a href={fileMeta.url} target="_blank" rel="noreferrer" className="underline">Ver</a> : <span>{fileMeta.filename || 'Archivo'}</span>}
                          <button
                            ...
                            onClick={async () => {
                              ...
                              if (fileMeta.url) {
                                try {
                                  const { getStorage, ref: sRef, deleteObject, refFromURL } = await import('firebase/storage');
                                  const storage = getStorage();
                                  let fileRef;
                                  try {
                                    fileRef = refFromURL(fileMeta.url);
                                  } catch (_) {
                                    try { fileRef = sRef(storage, fileMeta.url); } catch {}
                                  }
                                  if (fileRef) await deleteObject(fileRef).catch(()=>{ storageDeletedOk = false; });
                                } catch (e) {
                              ...
                              const next = { ...(legalProgress || {}) };
                              const prev = next[key];
                              if (typeof prev === 'object') { delete prev.file; next[key] = { ...prev }; }
                              setLegalProgress(next); saveLegalProgress(activeWedding, next);
                              try {
                                if (storageDeletedOk) toast.success(tr('documents.fileDeleted','Archivo eliminado'));
                                else toast.warn(tr('documents.fileRefRemoved','Referencia eliminada. No se pudo borrar del almacenamiento.'));
                              } catch {}
                            }}
                          >
                            Quitar
                      ...
                      <button
                        ...
                        onClick={() => {
                          try {
                            ...
                            try { toast.success(tr('documents.docDownloadStarted','Descarga iniciada (.DOC)')); } catch {}
                          } catch (e) {
                            try { toast.error(tr('documents.docFailed','No se pudo generar el .DOC')); } catch {}
                          }
                        }}
                      >
                      ...
                      <button
                        ...
                        onClick={async () => {
                          try {
                            ...
                            try { toast.success(tr('documents.pdfReady','PDF generado')); } catch {}
                          } catch (e) {
                            try { toast.error(tr('documents.pdfFailed','No se pudo generar el PDF')); } catch {}
                          }
                        }}
                      >
