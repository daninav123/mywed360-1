...
const AyudaCeremonia = () => {
  ...
  const [readings, setReadings] = useState([
    {
      id: 1,
      title: 'Lectura 1',
      content: 'Texto de la primera lectura...',
      duration: '3 min',
      status: 'draft',
    },
    {
  ...
  const handleReadingSave = (e) => {
    ...
    if (activeReading) {
      ...
      setReadings(
        readings.map((r) =>
          r.id === activeReading.id ? { ...r, title, content, status: 'draft' } : r
        )
      );
    } else {
      // Crear nueva lectura
            ...
            <div className="lg:col-span-1">
              <Card className="h-full">
                ...
                <div className="px-4 py-3 bg-gray-50 text-right">
                  <Button
                    onClick={() =>
                      setActiveReading({ id: null, title: '', content: '', status: 'draft' })
                    }
                    className="text-sm"
                  >
                    + Nueva Lectura
                              ...
                              onClick={() => {
                                ...
                                const updatedSurprises = surprises.map((s) =>
                                  s.id === surprise.id
                                    ? {
                                        ...s,
                                        status: s.status === 'pending' ? 'delivered' : 'pending',
                                      }
                                    : s
                                );
