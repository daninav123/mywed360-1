...
export default function Contratos() {
  ...
  } = useFirestoreCollection('contracts', sampleContracts);
  ...
  const [genericForm, setGenericForm] = useState({
    ...
    serviceDate: '',
    status: 'Vigente',
    templateId: 'contrato_generico_servicios',
  });
  ...
  const getTemplateById = (id) =>
    contractTemplates.find((t) => t.id === id) || contractTemplates[0];
  ...
  const actionButtons = (
    <>
      ...
      <button onClick={exportSelected} className="flex items-center">
  ...
  const __customVars = __vars.filter(
        ...
        <div className="hidden">
          <div className="bg-white p-6 rounded shadow w-96">
            ...
            >
              ...
              <input
                type="date"
                value={editContract.signedDate}
                onChange={(e) => setEditContract({ ...editContract, signedDate: e.target.value })}
                className="w-full border rounded px-2 py-1"
                required
              />
              <input
                type="date"
                value={editContract.serviceDate}
                onChange={(e) => setEditContract({ ...editContract, serviceDate: e.target.value })}
                className="w-full border rounded px-2 py-1"
                required
              />
              <select
          ...
          <div className="bg-white p-6 rounded shadow w-[90vw] max-w-3xl">
            ...
            <form onSubmit={handleGenerateGenericContracts} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setGenericForm({ ...genericForm, serviceDate: e.target.value })
                    }
                    className="w-full border rounded px-2 py-1"
                  />
                </div>
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={attachBaseDoc}
                  onChange={(e) => setAttachBaseDoc(e.target.checked)}
                />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {__customVars.length ? (
                    __customVars.map((k) => (
                      <div key={k}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setVariableValues({ ...variableValues, [k]: e.target.value })
                          }
                          className="w-full border rounded px-2 py-1"
                        />
                      </div>
                    ...
                    <div className="text-xs text-gray-500">
                      No hay variables adicionales en la plantilla.
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="hidden">
          <div className="bg-white rounded shadow-lg w-[90vw] max-w-3xl h-[85vh] relative">
            <div className="flex items-center justify-between p-3 border-b">
              ...
              <button className="px-2 py-1 border rounded" onClick={() => setShowPdfModal(false)}>
                Cerrar
              </button>
            </div>
  ...
  return (
    <div className="border border-[color:var(--color-text)]/15 rounded p-4 bg-[var(--color-surface)]">
      ...
      <BaseModal
        ...
        footer={
          <div className="flex justify-end gap-2">
            ...
            <Button form="add-contract-form" type="submit">
              Guardar
            </Button>
          </div>
        }
      ...
      <BaseModal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <Button variant="secondary" onClick={() => setShowGenericModal(false)}>
              Cancelar
            </Button>
            <Button form="generate-contracts-form" type="submit">
              Generar
            </Button>
          </div>
        }
      >
        <form
          id="generate-contracts-form"
          onSubmit={handleGenerateGenericContracts}
          className="space-y-4"
        >
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {__customVars.length ? (
                __customVars.map((k) => (
                  <div key={k}>
                    <label className="block mb-1 text-xs text-[color:var(--color-text)]/60">
                      {k}
                    </label>
                    <input
                      type="text"
          ...
          <div>
            ...
            <div className="border border-[color:var(--color-text)]/15 rounded p-2 bg-[var(--color-surface)] whitespace-pre-wrap text-sm font-mono max-h-56 overflow-auto">
              {previewContent || '...'}
            </div>
          </div>
          <div>
            ...
            <div className="border border-[color:var(--color-text)]/15 rounded max-h-64 overflow-auto divide-y divide-[color:var(--color-text)]/10">
              {providersLoading ? (
                <div className="p-3 text-sm text-[color:var(--color-text)]/60">
