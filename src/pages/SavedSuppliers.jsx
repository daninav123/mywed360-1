...
export default function SavedSuppliers() {
  ...
  const handleRemove = async (supplierId) => {
    if (!window.confirm(t('common.suppliers.favorites.confirmRemove'))) {
      return;
    }
    ...
    try {
      ...
      toast.error(error.message || t('common.suppliers.favorites.toasts.removeError'));
    } finally {
      setDeletingId(null);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        ...
        <span className="ml-3 text-lg">{t('common.suppliers.favorites.loading')}</span>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-2">
              ...
              {t('common.suppliers.saved.title')}
            </h1>
            <p className="text-gray-600 mt-2">
              {count === 0
                ? t('common.suppliers.saved.count.zero')
          ...
          {count > 0 && (
            ...
            >
              <Download size={18} />
              {t('common.suppliers.saved.buttons.export')}
            </button>
          )}
        </div>
      </div>
      ...
      {count === 0 ? (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">
            {t('common.suppliers.saved.empty.title')}
          </h3>
          <p className="text-gray-500 mb-6">{t('common.suppliers.saved.empty.description')}</p>
          <a
            href="/suppliers"
            className="inline-block px-6 py-3 bg-green-600 text-white rounded-md hover:bg-green-700 transition-colors"
          >
            {t('common.suppliers.saved.empty.cta')}
          </a>
        </div>
      ) : (
        <>
              ...
              <div key={favorite.id} className="relative">
                ...
                {favorite.addedAt && (
                  <div className="mt-2 text-xs text-gray-500">
                    {t('common.suppliers.saved.addedAt', {
          ...
          <div className="mt-8 p-6 bg-blue-50 border border-blue-200 rounded-lg">
            <h3 className="font-semibold text-lg mb-3">{t('common.suppliers.saved.tips.title')}</h3>
            <ul className="space-y-2 text-sm text-gray-700">
              {[
                t('common.suppliers.saved.tips.compare'),
                t('common.suppliers.saved.tips.contact'),
                t('common.suppliers.saved.tips.review'),
                t('common.suppliers.saved.tips.export'),
              ].map((tip) => (
                <li key={tip}>• {tip}</li>
              ))}
            </ul>
