...
const PaymentSuccess = () => {
  ...
  const statusLabel = useMemo(() => {
    ...
    if (sessionData.status === 'complete') {
      return t('common.public.payment.success.status.complete');
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">
            {t('common.public.payment.success.loadingMessage')}
          </p>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          <h2 className="mt-6 text-2xl font-semibold text-gray-900">
            {t('common.public.payment.success.errorTitle')}
          </h2>
          <p className="mt-2 text-gray-600">
            {t('common.public.payment.success.errorDescription')}
          </p>
          <div className="mt-8 space-y-3">
            ...
            >
              {t('common.public.payment.success.buttons.backPricing')}
            </Link>
            <Link
              to="/contact"
            ...
            >
              {t('common.public.payment.success.buttons.contactSupport')}
            </Link>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
        ...
        <h2 className="mt-6 text-2xl font-semibold text-gray-900">
          {t('common.public.payment.success.title')}
        </h2>

        <p className="mt-2 text-gray-600">
          {t('common.public.payment.success.description')}
        </p>

        {sessionData && (
          <div className="mt-6 bg-gray-50 rounded-lg p-4 text-left">
            <h3 className="text-sm font-semibold text-gray-700 mb-2">
              {t('common.public.payment.success.detailsHeading')}
            </h3>
            <dl className="space-y-1 text-sm">
              <div className="flex justify-between">
                <dt className="text-gray-500">
                  {t('common.public.payment.success.statusLabel')}
                </dt>
              ...
              {sessionData.amount_total && (
                <div className="flex justify-between">
                  <dt className="text-gray-500">
                    {t('common.public.payment.success.totalLabel')}
                  </dt>
        ...
        <div className="mt-8 space-y-3">
          <Link
            ...
            className="block w-full rounded-md bg-[var(--color-primary)] px-4 py-3 text-sm font-semibold text-white hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2"
          >
            {t('common.public.payment.success.buttons.goDashboard')}
          </Link>
          <p className="text-xs text-gray-500">
            {t('common.public.payment.success.redirectNotice')}
          </p>
        </div>

        <div className="mt-6 border-t border-gray-200 pt-6">
          <p className="text-sm text-gray-500">
            {t('common.public.payment.success.emailNotice')}
          </p>
        </div>
      </div>
    </div>
