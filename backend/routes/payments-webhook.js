...
router.post(
  '/webhook',
  express.raw({ type: 'application/json', limit: WEBHOOK_MAX_BYTES }),
  async (req, res) => {
    try {
      const disableSig =
        String(process.env.STRIPE_TEST_DISABLE_SIGNATURE || '').toLowerCase() === 'true';
      ...
      try {
        ...
        await db
          .collection('paymentsEvents')
          .add({ type: event.type, data: event.data?.object || {}, receivedAt: ts });
      } catch {}
      ...
      if (event.type === 'checkout.session.completed') {
          ...
          try {
            await db
              ...
              .set(
                {
                  paymentStatus: 'paid',
                  amountPaid: typeof amountTotal === 'number' ? amountTotal / 100 : null,
                  currency,
                  lastPaymentAt: tsNow,
                  updatedAt: tsNow,
                },
        ...
        if (providerId) {
          try {
            await db
              .collection('providerPayments')
              .doc(providerId)
              .set({ lastPaymentAt: tsNow, amount_total: amountTotal, currency }, { merge: true });
          } catch {}
          if (weddingId) {
            try {
              await db
                ...
                .doc(providerId)
                .set({ status: 'Señal pagada', updatedAt: tsNow }, { merge: true });
            } catch (e) {
        ...
        if (contractId) {
          try {
            await db
              ...
              .doc(contractId)
              .set(
                { paymentStatus: 'paid', lastPaymentAt: tsNow, updatedAt: tsNow },
                { merge: true }
              );
          } catch {}
        }
            ...
            .set(
              {
                kind: 'payment_intent',
                status: 'failed',
                error: obj.last_payment_error || null,
                updatedAt: tsNow,
                contractId,
              },
              { merge: true }
        ...
        if (contractId) {
          try {
            await db
              .collection('contracts')
              .doc(contractId)
              .set({ paymentStatus: 'failed', updatedAt: tsNow }, { merge: true });
          } catch {}
        }
      }
