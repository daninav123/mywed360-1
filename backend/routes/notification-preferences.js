...
router.get('/', async (req, res) => {
  try {
    ...
    if (!uid)
      return res
        .status(401)
        .json({ success: false, error: { code: 'unauthenticated', message: 'Auth required' } });
    const snap = await db.collection('notificationPreferences').doc(uid).get();
    const data = snap.exists
      ? snap.data()
      : { channels: { email: true, inapp: true, push: false }, quietHours: null };
...
router.put('/', async (req, res) => {
  try {
    ...
    await db
      ...
      .doc(uid)
      .set(
        { channels, quietHours, updatedAt: admin.firestore.FieldValue.serverTimestamp() },
        { merge: true }
      );
