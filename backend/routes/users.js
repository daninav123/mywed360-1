...
router.post('/upgrade-role', async (req, res) => {
  try {
    ...
    const rawRole = String(req.body?.newRole || '').trim().toLowerCase();
    const allowed = new Set(['owner', 'assistant', 'planner']);
    if (!allowed.has(rawRole)) {
      return res.status(400).json({ success: false, error: { code: 'bad_request', message: 'newRole inválido' } });
    }

    const tier = String(req.body?.tier || '').trim() || (rawRole === 'planner' ? 'wedding_planner_1' : (rawRole === 'assistant' ? 'assistant' : 'free'));
    ...
    if (rawRole === 'planner') {
      payload.plannerWeddingIds = Array.isArray(before.plannerWeddingIds) ? before.plannerWeddingIds : [];
    }

    // Persistir cambios
    await userRef.set(payload, { merge: true });
    try {
      await userRef.set({ roleHistory: admin.firestore.FieldValue.arrayUnion(roleHistoryEntry) }, { merge: true });
    } catch {}
