...
router.post('/', express.json({ limit: '50kb' }), async (req, res) => {
  try {
    ...
    for (const m of list) {
      ...
      logger.info(
        `[web-vitals] ${name}=${value} delta=${delta} id=${id} label=${label} nav=${navigationType || ''}`
      );
      // Buffer for admin queries (best-effort)
      try {
        recentWebVitals.push({
          ts: Date.now(),
          name,
          value,
          id,
      ...
      try {
        ...
        if (c)
          c.labels({ name: String(name || 'unknown'), label: String(label || 'unknown') }).inc(1);
        ...
        if (h && typeof value === 'number')
          h.labels({ name: String(name || 'unknown'), label: String(label || 'unknown') }).observe(
            value
          );
      } catch {}
    }
