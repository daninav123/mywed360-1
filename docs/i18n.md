# Internacionalización (i18n) - MaLoveApp

## Resumen

MaLoveApp ahora soporta internacionalización completa utilizando **react-i18next**, permitiendo que la aplicación sea accesible globalmente en múltiples idiomas. La implementación incluye traducciones, formateo localizado, selector de idioma y arquitectura escalable para futuras expansiones.
...
│       └── en/
│           └── common.json      # Base en inglés (fallback para nuevos idiomas)
├── components/ui/
│   └── LanguageSelector.jsx     # Componente selector de idioma
└── hooks/
    └── useTranslations.js       # Hook personalizado para traducciones
```
> Los idiomas adicionales reutilizan de momento las cadenas en inglés (o español, para variantes `es-*`) hasta completar sus traducciones dedicadas; esto permite habilitarlos en UX y planificar la localización progresiva.

### Idiomas disponibles en la app

| Código | Idioma | Región | Estado |
|--------|--------|--------|--------|
| `es` | Español (España) | España | ✅ Completo (idioma base) |
| `es-MX` | Español (México) | México | ✅ Habilitado (fallback en español) |
| `es-AR` | Español (Argentina) | Argentina | ✅ Habilitado (fallback en español) |
| `en` | Inglés | Reino Unido / EE. UU. | ✅ Completo |
| `fr` | Francés | Francia | ✅ Habilitado (fallback en inglés) |
| `fr-CA` | Francés (Canadá) | Quebec / Canadá | ✅ Habilitado (fallback en francés FR) |
| `de` | Alemán | Alemania | ✅ Habilitado (fallback en inglés) |
| `it` | Italiano | Italia | ✅ Habilitado (fallback en inglés) |
| `pt` | Portugués (Brasil) | Brasil / Portugal | ✅ Habilitado (fallback en inglés) |
| `nl` | Neerlandés | Países Bajos / Bélgica | ✅ Habilitado (fallback en inglés) |
| `pl` | Polaco | Polonia | ✅ Habilitado (fallback en inglés) |
| `fi` | Finés | Finlandia | ✅ Habilitado (fallback en inglés) |
| `is` | Islandés | Islandia | ✅ Habilitado (fallback en inglés) |
| `sv` | Sueco | Suecia | ✅ Habilitado (fallback en inglés) |
| `da` | Danés | Dinamarca | ✅ Habilitado (fallback en inglés) |
| `no` | Noruego | Noruega | ✅ Habilitado (fallback en inglés) |
| `lv` | Letón | Letonia | ✅ Habilitado (fallback en inglés) |
| `lt` | Lituano | Lituania | ✅ Habilitado (fallback en inglés) |
| `et` | Estonio | Estonia | ✅ Habilitado (fallback en inglés) |
| `cs` | Checo | República Checa | ✅ Habilitado (fallback en inglés) |
| `sk` | Eslovaco | Eslovaquia | ✅ Habilitado (fallback en inglés) |
| `hu` | Húngaro | Hungría | ✅ Habilitado (fallback en inglés) |
| `ro` | Rumano | Rumanía | ✅ Habilitado (fallback en inglés) |
| `bg` | Búlgaro | Bulgaria | ✅ Habilitado (fallback en inglés) |
| `sl` | Esloveno | Eslovenia | ✅ Habilitado (fallback en inglés) |
| `hr` | Croata | Croacia | ✅ Habilitado (fallback en inglés) |
| `el` | Griego | Grecia | ✅ Habilitado (fallback en inglés) |
| `mt` | Maltés | Malta | ✅ Habilitado (fallback en inglés) |
| `tr` | Turco | Turquía | ✅ Habilitado (fallback en inglés) |
| `ar` | Árabe | Oriente Medio / diáspora | ✅ Habilitado (fallback en inglés) |
| `ru` | Ruso | Europa del Este / Asia Central | ✅ Habilitado (fallback en inglés) |
| `ca` | Catalán | Cataluña / Andorra | ✅ Habilitado (fallback en inglés) |
| `eu` | Euskera | País Vasco / Navarra | ✅ Habilitado (fallback en inglés) |

### Próximos idiomas objetivo (impacto global sin foco en África)

| Código | Idioma | Mercado objetivo | Prioridad |
|--------|--------|------------------|-----------|
| `zh-CN` | Chino simplificado | China continental, comunidad global | 🟡 Planificado |
| `zh-TW` | Chino tradicional | Taiwán / Hong Kong | 🟡 Planificado |
| `ja` | Japonés | Japón | 🟡 Planificado |
| `ko` | Coreano | Corea del Sur | 🟡 Planificado |
| `hi` | Hindi | India | 🟡 Planificado |
| `id` | Indonesio | Sudeste asiático (Indonesia) | 🟡 Planificado |
| `th` | Tailandés | Tailandia (turismo bodas) | 🟡 Planificado |
| `vi` | Vietnamita | Vietnam | 🟡 Planificado |
| `fil` | Filipino / Tagalo | Filipinas y diáspora | 🟡 Planificado |
| `he` | Hebreo | Israel | 🟡 Planificado |
| `fa` | Persa | Irán y diáspora | 🟡 Planificado |
| `uk` | Ucraniano | Europa del Este | 🟡 Planificado |
| `sr` | Serbio (latino) | Balcanes | 🟡 Planificado |
| `mk` | Macedonio | Balcanes | 🟡 Planificado |
| `sq` | Albanés | Balcanes / diáspora UE | 🟡 Planificado |
| `en-AU` / `en-NZ` | Inglés regional | Oceanía (formatos locales) | 🟢 Ajuste de formatos |

> Cada nuevo idioma requiere glosario, validación cultural y soporte continuo. Se priorizarán Asia-Pacífico, oriente medio y Balcanes según las integraciones comerciales previstas.
...
Hook personalizado que extiende `useTranslation` con funcionalidades específicas de MaLoveApp:

```javascript
const { t, format, wedding, getNavigation } = useTranslations();

// Traducción básica
const title = t('app.name'); //  – MaLoveApp – 
...
### Fase 2: Expansión 🔄
- [ ] Traducciones francés/alemán/italiano
...
**Versión:** 1.0.0  
**Mantenedor:** Equipo de Desarrollo MaLoveApp
