...
3. **Inspiración reciente**
   - Carga inicial (`useEffect`) que consulta `fetchWall` para las categorías `decoración`, `ceremonia`, `banquete`, `flores`, `vestidos`; se muestra skeleton accesible mientras llega la respuesta.
...
- **Alertas**: `usePlannerAlerts` agrega notificaciones sin leer para las bodas del planner filtrando `payload.weddingId`; escucha `window` `MaLove.App-notif` para refrescos rápidos.
- **Notificaciones**: `notifications` (colección global + cache local) guarda `payload` enriquecido (weddingId, category, severity, source). Opción: contadores agregados `weddings/{id}/counters/alerts` para evitar agregados costosos.
