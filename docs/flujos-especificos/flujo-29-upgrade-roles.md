...
- Garantizar que la navegación, dashboards y permisos reflejan el nuevo rol inmediatamente tras el upgrade.
...
4. El usuario recibe instrucciones para solicitar invitación; `WeddingAccountLink` añade campo “Código de invitación” visible para assistants.
5. Al introducir código, se llama `acceptInvitation(code)` (`src/services/WeddingService.js:510`) y se agrega el uid a `assistantIds`. Se crea/actualiza enlace en `users/{uid}/weddings/{weddingId}` con rol `assistant`.
6. `WeddingContext` detecta la nueva entrada y actualiza `activeWedding`. Nav pasa a layout assistant (`src/components/Nav.jsx:69`), sin acceso a finanzas.
...
2. Modal muestra límites del plan (5/10/∞ bodas según tier) y acceso al `PlannerDashboard`.
...
- `users/{uid}`:
  - Campos `role`, `subscription.tier`, `activeWeddingId`, `plannerWeddingIds`, `hasActiveWedding` (ya usados en `WeddingService.js:219` y `WeddingService.js:554`).
...
- Analytics: `performanceMonitor.logEvent('wedding_switched', ...)` ya registra cambios de boda; añadir `role_upgraded` y `role_downgraded`.
...
- **Regression**: asegurar que owners sin upgrade continúan viendo su dashboard habitual y no aparecen CTAs erróneos.
