...
3. Calendario y automatización
   ...
   - Gantt por bloques (vista global de la boda):
     ...
     - El timeline debe ajustarse para que el rango `weddingDate - 12 meses` → `weddingDate` encaje en ancho de pantalla estándar (1366px / 1440px) sin necesidad de scroll horizontal. El scroll horizontal solo se habilitará como fallback cuando la densidad de texto impida la lectura; en ese caso se mostrará un indicador visual (sombra/fade) para señalar que hay contenido adicional.
...
- Las subtareas no requieren fecha exacta: pueden permanecer solo con ventana orientativa siempre que el trabajo quede dentro del rango de la tarea padre. Si se agenda una fecha concreta, debe respetar los límites `start/end` del bloque principal.
