...
## 2. Trigger y rutas
- Men� inferior � pesta�a **Inicio** (`/home`, `Dashboard.jsx`) como landing por defecto.
- Men� inferior � `M�s` (`/more`, `More.jsx`) muestra accesos a invitados, proveedores, protocolo y extras.
- Men� de usuario � `/perfil` (`Perfil.jsx`) para preferencias, idioma, plan y seguridad.

## 3. Paso a paso UX
1. Dashboard principal
   - Widgets configurables (progreso checklist, presupuesto, invitados, timeline, comunicacion) m�s nuevos m�dulos `Mapa de preferencias` y `Salud del perfil` (StyleMeter, alertas de contraste, gaps).
   ...
   - Tarjetas num�ricas de resumen (ver detalle m�s abajo) para invitados confirmados, presupuesto, proveedores y tareas.
2. Navegacion secundaria
   ...
   - Buscador global (pendiente). Nota 2025-10-12: el componente `GlobalSearch` solo aparece en desktop (header) y se abre con bot�n, sin atajos (Ctrl/Cmd+K).
...
- Consume estad�sticas de flujos 2C,3,4,5,6,7,9,14,17 para widgets (mapa de preferencias, checklist, presupuesto, comunicaci�n).
...
- Cuando faltan datos, se renderiza `0` (o `0 / 0`). Al conectar con Firestore, se sustituir� el fallback `localStorage` por consultas reales y se a�adir�n estados vac�os descriptivos.

### Acciones r�pidas (HomePage)
...
- Los botones ya no ofrecen atajos Ir a &; cada acci�n se resuelve en el mismo modal.  
...
- Estas acciones r�pidas sirven como atajos operativos; la gesti�n completa sigue disponible en los m�dulos dedicados a trav�s de la navegaci�n principal.
...
- **Datos que consume**:
  ...
  - Fecha objetivo de la boda (`weddingDate`, `date`, `eventDate` o `ceremonyDate`) y punto de inicio (`planningStart`, `planningStartDate`, `projectStart`, `onboardingCompletedAt`). Si no hay fechas v�lidas se usa `eventDate - 365 d�as` como inicio.
- **Persistencia**: el porcentaje calculado se guarda en `localStorage.maloveapp_progress` como fallback cuando no hay conexi�n (el texto No pudimos sincronizar el avance usa este valor).
- **Estados visuales**:
  - `success`: percent e 100 o >5 puntos por encima del progreso esperado.
  - `warning`: dentro del rango �5 del progreso esperado.
  - `destructive`: m�s de 5 puntos por debajo del esperado o sin referencia temporal con progreso bajo.
- **Mensajes**:
  - `Vas adelantado al plan previsto` / `Revisa tareas clave para ponerte al d�a` / `Progreso sin referencia temporal`.
  - Cuando `expectedProgress` es `null` (no hay fechas) se oculta la comparaci�n Esperado: X%.
- **Siguientes pasos**: migrar el c�lculo de puntos al backend (GamificationService) y exponer endpoints con metas diferenciadas por plan. A�adir CTA contextual (abrir checklist, sugerir tareas) cuando se detecte retraso.
...
- No hay pruebas end-to-end espec�ficas implementadas para este flujo.
...
- `HomePage.jsx` muestra el progreso de checklist y la informaci�n de `GamificationPanel` dentro de la misma tarjeta, ocultando el panel cuando no hay datos de gamificaci�n disponibles; a�n carece de widgets configurables o m�tricas en vivo.
- `Dashboard.jsx` funciona con widgets drag-and-drop pero la aplicaci�n no navega a este layout por defecto (landing usa `HomePage`).
- `GlobalSearch` requiere click manual; no hay shortcuts ni resultados mockeados para tests.
- Herramientas de diagn�stico (`DevEnsureFinance`, `DevSeedGuests`) est�n ocultas en builds por defecto; no hay panel �nico ni m�tricas UI visibles.
...
- Exponer men� o bot�n para abrir herramientas de diagn�stico con m�tricas b�sicas (ej. totales finance/guests) visibles en DOM.
- A�adir shortcut accesible (`Ctrl/Cmd+K`) para `GlobalSearch` y resultados deterministas en modo E2E.
- Identificar breadcrumbs o breadcrumbs virtuales (`data-testid="breadcrumb"`) en cada navegaci�n cr�tica (Home, Tasks, Finance, More).
...
2. `global-search-shortcuts`: enviar `cy.realPress(['ctrl','k'])`, escribir t�rmino mock y navegar al primer resultado asegurando ruta correcta.
3. `diagnostic-panel`: abrir panel dev, validar presencia de m�tricas (`data-testid="diag-total-guests"` etc.) y cerrarlo.
...
- Flag `VITE_ENABLE_DEV_TOOLS` expuesto en entorno de prueba para mostrar panel de diagn�stico.
