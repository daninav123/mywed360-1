...
> Implementado: `HomeUser.jsx`, `Dashboard.jsx`, `More.jsx`, `Perfil.jsx`, widgets `WidgetContent.jsx`, utilidades de diagnóstico (`DevEnsureFinance.jsx`, `DevSeedGuests.jsx`).
> Pendiente: unificar dashboard con métricas en vivo, proteger herramientas internas y agregar actividad reciente + estado de sincronización.

## 1. Objetivo y alcance
- Ofrecer panel de control central con accesos rápidos a módulos y resumen de estado.
- Gestionar navegacion global, perfil del usuario y recursos de soporte.
- Facilitar diagnóstico en ambientes internos con herramientas de seed (solo dev).
...
1. Dashboard principal
   ...
   - Tarjetas numricas de resumen (ver detalle más abajo) para invitados confirmados, presupuesto, proveedores y tareas.
...
- Cambios de plan requieren confirmación y sincronización con billing.

## 6. Estados especiales y errores
- Sin boda activa -> mostrar wizard onboarding (Flujo 2).
- Faltan permisos -> ocultar módulos y mostrar mensaje "No tienes acceso a...".
...
- Consume estadsticas de flujos 2C,3,4,5,6,7,9,14,17 para widgets (mapa de preferencias, checklist, presupuesto, comunicación).
...
### Tarjetas de métricas en Home (`HomePage.jsx`)
...
|-------|--------|-----------------------|--------------------------|---------------|
| Buscar proveedor | Abre `ProviderSearchModal`, permite buscar por servicio/presupuesto y seleccionar un resultado. | Guarda el proveedor elegido en `localStorage.lovendaProviders`, emite `MaLove.App-providers` y muestra `toast.success`. | Migrará a `weddings/{id}/suppliers` con seeds propios. | Se eliminó el CTA “Ir a proveedores”; todo sucede dentro del modal. |
| Añadir invitado | Captura nombre, parte y contacto en un formulario liviano. | Inserta el registro en `localStorage.MaLove.AppGuests` y muestra `toast.success`. | Futuro: creación directa en `weddings/{id}/guests`. | No redirige; pensado para alta rápida. |
...
- **Archivo**: `HomePage.jsx`, sección `progressPercent` y helper `computeExpectedProgress`.
- **Datos que consume**:
...
- Identificar breadcrumbs o breadcrumbs virtuales (`data-testid="breadcrumb"`) en cada navegación crtica (Home, Tasks, Finance, More).
...
2. `global-search-shortcuts`: enviar `cy.realPress(['ctrl','k'])`, escribir trmino mock y navegar al primer resultado asegurando ruta correcta.
