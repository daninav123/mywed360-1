...
### 1️⃣ **Hook Personalizado: `useTaskDependencies`**
...
const TaskList = ({ 
  ...
  completedSet, 
  onToggleComplete, 
  dependencyStatuses = new Map() // 👈 Nuevo
}) => {
```
...
```
Usuario: [Intenta marcar checkbox de "Contratar fotógrafo"]
Sistema: 🔒 No puedes completar esta tarea aún.

         Debes completar primero: Elegir fotógrafo
[OK]
```
...
```
Usuario: [Completa "Elegir fotógrafo" ✓]
Sistema: 🎉 ¡Excelente! Ahora puedes trabajar en: 
         Contratar fotógrafo, Sesión pre-boda
...
getUnblockedTasks(justCompletedTaskId) {
  ...
  for (const task of tasks) {
    // 1. ¿Esta tarea dependía de la recién completada?
    ...
    // 2. Crear nuevo Set con la tarea recién completada
...
Map {
  "task-123" => {
    isBlocked: true,
    missingDeps: [
      { taskId: "task-456", taskTitle: "Elegir fotógrafo", isCompleted: false }
    ],
    completedDeps: [],
    allDeps: [...]
  }
...
3. Intentar completar B → ❌ Bloqueada
...
### **Fotografía:**
```
1. Elegir fotógrafo (sin deps)
2. Contratar fotógrafo (depende de #1)
3. Sesión pre-boda (depende de #2)
4. Álbum final (depende de #3)
```
