...
> **Actualizaci�n 2025-10-22:** Actualmente 13 suites `*-real.cy.js` ejecutan integraciones reales (Auth, Dashboard, Guests y Email). El resto de suites listadas a continuaci�n siguen dependiendo de interceptores/mocks legacy. Mientras se completa la migraci�n, utiliza la tabla siguiente �nicamente como referencia documental y consulta la secci�n "Cobertura con integraci�n real" para conocer qu� flujos ya se validan con datos reales.

## Cobertura con integraci�n real (2025-10-22)

| M�dulo | Suites `*-real.cy.js` | Alcance cubierto |
|--------|----------------------|------------------|
| Auth | `cypress/e2e/auth/auth-flow-real.cy.js`<br>`cypress/e2e/auth/flow1-signup-real.cy.js`<br>`cypress/e2e/auth/flow1-social-login-real.cy.js`<br>`cypress/e2e/auth/flow1-password-reset-real.cy.js`<br>`cypress/e2e/auth/flow1-verify-email-real.cy.js`<br>`cypress/e2e/critical/auth-real.cy.js` | Login persistente, social login (manejo real de errores), rutas protegidas, signup, reset y verificaci�n de email con Firebase real |
...
| 11. Protocolo y Ceremonias (visi�n global) | cypress/e2e/protocolo/ceremony-tabs-flow.cy.js |  | Legacy (mocks) |
| 11A. Momentos Especiales de la Boda | cypress/e2e/protocolo/protocolo-flows.cy.js |  | Legacy (mocks) |
| 11B. Timeline Global del D�a B | cypress/e2e/protocolo/protocolo-flows.cy.js |  | Legacy (mocks) |
| 11C. Checklist de �ltima Hora | cypress/e2e/protocolo/protocolo-flows.cy.js |  | Legacy (mocks) |
| 11D. Gu�a de Documentaci�n Legal | cypress/e2e/protocolo/protocolo-flows.cy.js |  | Legacy (mocks) |
...
| 27. Momentos (�lbum Compartido)  estado 2025-10-15 | cypress/e2e/moments/moments-empty-state.cy.js |  | Legacy (mocks) |
...
| 2C. Personalizaci�n IA Continua (estado 2025-10-14) | cypress/e2e/personalization/personalization-preferences.cy.js |  | Legacy (mocks) |
...
_Nota:_ La matriz legacy refleja lo declarado en la documentaci�n de cada flujo (`## Cobertura E2E implementada`). Si el flujo no menciona ninguna spec, aparecer� como �Sin specs en doc�. Todas estas suites (sin sufijo `-real.cy.js`) dependen de interceptores y mocks hasta completar la migraci�n.
