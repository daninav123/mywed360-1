...
- Por dominios (feature-first):
  - `apps/main-app/src/features/tasks/{components,hooks,services,types}`
  - `apps/main-app/src/features/email/...`
  - `apps/main-app/src/components/ui` (kit reutilizable: Button, Input, Modal, Table, EmptyState, Loader)
- Reglas
...
## UI (Tokens CSS + Tailwind + MUI)
- Principios
  - Layout y espaciado con Tailwind, componentes complejos con MUI cuando aporte accesibilidad (pickers/tablas).
  - Tokens coherentes: usar variables CSS (`--color-*`) definidas en `apps/*/src/index.css`.
- Tokens recomendados (Tailwind)
  - Tipografía: `font-sans: Inter`; escala sugerida: `sm, base, lg, xl, 2xl, 3xl`.
- Colores: usar tokens `--color-*` en `apps/*/src/index.css` (por defecto: `success (#22c55e)`, `warning (#f59e0b)`, `danger (#ef4444)`, `info (#0ea5e9)`).
...
- Layout responsivo:
  - Variables CSS (`apps/*/src/index.css`): `--layout-max-width: 1120px`, `--layout-wide-width: 1280px`, `--layout-padding: clamp(16px, 4vw, 32px)`.
...
- Patrones (inspirados en Home/Tasks)
  - Cards con `rounded-xl bg-surface border border-soft shadow-md` y `p-4/6`.
...
- Tokens CSS (definidos en `apps/*/src/index.css`):
...
- Utilidades semánticas para migración gradual (definidas en `@layer utilities` en `apps/*/src/index.css`):
...
- `Card` (`apps/main-app/src/components/ui/Card.jsx`): contenedor estándar con tokens, borde suave y sombra.
- `Button` (`apps/main-app/src/components/ui/Button.jsx`): variantes `primary|secondary|outline|ghost|destructive|link`, tamaños `xs..xl`.
- `Input` (`apps/main-app/src/components/Input.jsx`): con etiqueta, error, y foco consistente.
- `Modal` (`apps/main-app/src/components/Modal.jsx`) actualizado: tokens, accesible y tamaños `sm|md|lg|xl|full`.
- `Tabs` (`apps/main-app/src/components/ui/Tabs.jsx`) actualizado: indicador inferior con `--color-primary` y texto inactivo atenuado.
- `Loader` (`apps/main-app/src/components/ui/Loader.jsx`) actualizado: colores por tokens.
...
3) Para nuevas piezas, partir de Home/Tasks como referencia visual: uso de `var(--color-*)` y fondos sutiles (`--color-surface-70/80`). Evitar blur/gradientes.
...
- Tamaño: referencia Tasks → `text-2xl` con `font-semibold` y `mb-6` (ver `h1, .page-title` en `apps/*/src/index.css`).
...
- Componente listo: `apps/main-app/src/components/ui/PageTabs.jsx`
